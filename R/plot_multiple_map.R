...
#' @description outputs multiple choropleth map plots, each representing one
#' variable.
#' @details variable names and palette can be customized for each plot,
#' otherwise parameters will be the same for each variable plotted
...
#' same length as the number of covariates and cases specified  in 'var'
#' arguments.
...
.plot_multiple_map <- function(
  ...
  if (!is.null(palette)) {
    ...
    if (!(length(palette) == 1 || length(palette) == expected_length)) {
      stop(paste(
        "'palette' must be either length 1 (same for all plots) or match",
