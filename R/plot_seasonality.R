#' Seasonality plot
#'
#' @description Plots yearly time series of covariates, case counts, or
...
#' @param aggregate_space Name of variable used to define spatial aggregation
#' groups.
...
plot_seasonality <- function(
    ...
    if (!is.null(var_label)) {
      ...
      if (type == "cov") {
        ...
        if (is.null(aggregate_time) && !is.null(area)) {
          ...
          time_interval <- .get_time_interval(data = data, time = "time")
        } else {
  ...
  out <- out + ggplot2::scale_color_manual(values = my_palette, name = "Year")
