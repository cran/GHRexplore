#' @title Combine plots
#'
#' @description Combines plots, each representing one variable, into a
#' single plot.
#' @details This function takes any input from the `cowplot::plot_grid`
...
#' number of rows in which to align plots and the common legend. Default is
#' NULL.
#' @param rel_widths_l When combine_legend = TRUE, vector of widths in which to
#' align plots and the common legend. Default is c(3, 1).
#' @param rel_heights_l When combine_legend = TRUE, vector of heights in which
#' to align plots and the common legend. Default is c(1, 1).
...
plot_combine <- function(
  ...
  if (combine_legend) {
    ...
    if (
      !is.numeric(rel_widths_l) ||
        length(rel_widths_l) != 2 ||
        any(rel_widths_l <= 0)
    ) {
      stop(
        "rel_widths_l must be a numeric vector of length 2 with positive values"
      )
    }
    if (
      !is.null(rel_heights_l) &&
        (!is.numeric(rel_heights_l) ||
          length(rel_heights_l) != 2 ||
          any(rel_heights_l <= 0))
    ) {
      stop(
        "rel_heights_l must be a numeric vector of ",
        "length 2 with positive values"
      )
    }
  }
  ...
  # Remove x-axis from all but the last plot
  # (if plots are aligned in one column)
  if (combine_xaxis == TRUE && ncol == 1) {
