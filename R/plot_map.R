...
#' @param pop Character identifying the variable name for population. Only
#' needed if `type='inc'`.
...
#' name as in `area`.
#' @param aggregate_time Temporal scale for visualization and aggregation.
#' Options include "all" (across all time points) and "year" (default).
...
#' any of the in-built ggplot2 options such as  "log10" `log10(x)`, "log1p"
#' `log(x+1)`,  and "sqrt" `sqrt(x)` (check all possible options using
#' `?scale_y_continuous`).
#' @param title Optional title of the plot.
#' @param var_label Character with a custom name for the case or covariate
#' variable.
#' @param palette GHR, RColorBrewer or colorspace palette. Use "-" before
...
#' @param bins_method Method to compute the bins, only used when `bins`
#' is not NULL.  Possible values are "quantile" (default) and "equal".
#' @param bins_label Optional labels for the bins. They must have the
#' same length as the number of bins. Defaults to NULL (default interval
#' labels).
...
plot_map <- function(
  ...
  if (!missing(by_year)) {
    stop(paste0(
      "The argument 'by_year' has been deprecated. Please use ",
  ...
  if (!transform %in% val_trans) {
    ...
    stop(
      "Transform log10p1 is only available for case counts and incidence rates."
    )
  }
  ...
  if (length(missing_in_data) > 0 || length(missing_in_map) > 0) {
    if (length(missing_in_data) > 0) {
      warn1 <- paste0(
        "Values in 'map' not found in 'data': ",
    ...
    if (length(missing_in_map) > 0) {
      warn2 <- paste0(
        "Values in 'data' not found in 'map': ",
  ...
  if (type == "cov") {
    ...
    if (
      !class(data[[var]]) %in% c("integer", "numeric", "factor", "character")
    ) {
    ...
    if (class(data[[var]]) %in% c("factor", "character")) {
      ...
      if (aggregate_time == "all") {
        ...
        if (any(data_distinct$n_unique > 1)) {
          stop(paste0(
            "plot_map does not accept time-varying categorical covariates. ",
      ...
      if (aggregate_time == "year") {
        ...
        data_agg <- data |>
          ...
          dplyr::summarise(
            var = match.fun(aggregate_time_fun)(
              !!rlang::sym(var),
              na.rm = TRUE
            ),
            .groups = "drop"
      ...
      if (aggregate_time == "year") {
        ...
        map_expanded <- map |>
          dplyr::rowwise() |>
          dplyr::mutate(year = list(all_years)) |>
          # Attach the list of years to each row
          tidyr::unnest(year)
    ...
    if (aggregate_time == "year") {
      ...
      # Create all combinations of map rows and the years we have in data
      # NOTE: This effectively replicates each row of 'map' for every year
      map_expanded <- map |>
        dplyr::rowwise() |>
        dplyr::mutate(year = list(all_years)) |>
        # Attach the list of years to each row
  ...
  if (is.factor(map_data[["plot_var"]])) {
    # Categorical variable and bins

    level_names <- unique(map_data$plot_var[!is.na(map_data$plot_var)])
    my_palette <- GHR_palette(palette, length(level_names))(
    ...
    rescaled_center <- (centering - limit[["min"]]) /
      (limit[["max"]] - limit[["min"]])
    values_pos <- c(
      ...
      out <- out +
        ...
        ggplot2::scale_color_manual(
          ...
          guide = ggplot2::guide_legend(
            override.aes = list(fill = "grey50", color = "grey20")
          )
        ) +
        ggplot2::labs(color = "") +
        ggplot2::guides(
