...
test_that("basic input validation works", {
  ...
  expect_error(
    plot_timeseries2(data = df, var = c("x", "y")),
    "Missing required argument 'time'"
  )
  expect_error(
    plot_timeseries2(data = "notdf", var = c("x", "y"), time = "date"),
    "should be a data.frame"
  )
  expect_error(
    plot_timeseries2(df, var = "x", time = "date"),
    "'var' must have length 2"
  )
  expect_error(
    plot_timeseries2(df, var = c("x", "notacol"), time = "date"),
    "do not include the 'var'"
  )
  expect_error(
    plot_timeseries2(
      df,
  ...
  expect_error(
    plot_timeseries2(df, var = c("x", "y"), time = "date", align = "bad"),
    "'align' must"
  )
  expect_error(
    plot_timeseries2(
