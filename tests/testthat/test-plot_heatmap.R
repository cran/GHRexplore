...
test_that("plot_heatmap handles missing arguments correctly", {
  expect_error(plot_heatmap(), "Missing required argument 'data'")
  expect_error(plot_heatmap(data), "Missing required argument 'var'")
})
...
test_that("plot_heatmap handles incorrect argument types", {
  expect_error(
    plot_heatmap(data = list(), time = "time", var = "cases"),
    "'data' should be a data.frame"
  )
  expect_error(
    plot_heatmap(data, time = "invalid_column", var = "cases"),
    "'time' not found in the data"
  )
  expect_error(
    plot_heatmap(data, time = "time", var = "var", aggregate_time = "invalid"),
    "'aggregate_time' can be 'week' or 'month'"
  )
})

test_that("plot_heatmap correctly processes valid input", {
  expect_silent(plot_heatmap(
    data,
    time = "time",
    var = "cases",
    type = "counts",
  ...
  expect_silent(plot_heatmap(data, time = "time", var = "var", area = "area"))
})
