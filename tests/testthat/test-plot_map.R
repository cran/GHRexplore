...
testdata <- data.frame(
  ...
  area = c("50001", "50002", "50003", "50001", "50002", "50003"),
  cat_char = LETTERS[1:6],
  cat_factor = as.factor(LETTERS[1:6]),
  cases = c(10, 20, 30, 10, 20, 30),
...
test_that("plot_map requires time to be a date when aggregate_time='year'", {
  testdata_wrongtime <- testdata
  # Convert to incorrect format
  testdata_wrongtime$time <- as.character(format.Date(
    testdata$time,
    "%d-%m-%Y"
  ))
  expect_error(
...
test_that("plot_map requires a valid area column", {
  expect_error(
    plot_map(testdata, var = "cases", time = "time", area = "invalid"),
...
test_that("plot_map requires a valid map argument", {
  expect_error(
    plot_map(testdata, var = "cases", time = "time", area = "area"),
