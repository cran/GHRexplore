test_that("plot_seasonality handles missing or invalid data", {
  ...
  expect_error(
    plot_seasonality(data = list(time = as.Date("2025-01-01"), cases = 10)),
...
test_that("plot_seasonality aggregates data correctly", {
  ...
  data <- data.frame(
    time = as.Date(c("2025-01-01", "2025-02-01", "2025-01-01", "2025-02-01")),
    var = c(1, 2, 3, 1),
    area = rep(letters[1:2], each = 2)
  )
...
test_that("Error when population is missing for incidence", {
  ...
  expect_error(
    plot_seasonality(data = data, var = "cases", time = "time", type = "inc"),
