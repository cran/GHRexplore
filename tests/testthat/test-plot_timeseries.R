...
test_that("'data' should be a data.frame", {
  expect_error(
    plot_timeseries(data = list(a = 1), time = "date"),
...
test_that("Error when 'time' is missing", {
  ...
  expect_error(
    plot_timeseries(data = data, var = "a"),
...
test_that("'Date' format is invalid", {
  ...
  expect_error(
    plot_timeseries(data = data, var = "a", time = "time"),
...
test_that("Error when 'var' is missing", {
  ...
  expect_error(
    plot_timeseries(data = data, time = time),
    "Error: Missing required argument 'var'"
  )
})
...
test_that("No column matches the 'area' argument", {
  data <- data.frame(time = as.Date("2025-01-01"), a = 1)
  expect_error(
    plot_timeseries(data = data, time = "time", area = "region", var = "a"),
...
test_that("Missing values in 'var'", {
  ...
  # Match only the warning related to missing values in 'cases', ignore others.
  expect_warning(
...
test_that("Error when 'var' is not found in the data", {
  ...
  expect_error(
    plot_timeseries(data, var = "missing_var", time = "time"),
...
test_that("'var' should be numeric", {
  ...
  expect_error(
    plot_timeseries(data, var = "value", time = "time"),
...
test_that("Duplicated time units detected within the same 'area'", {
  data <- data.frame(
    time = rep(as.Date("2025-01-01"), 2),
    value = 5,
    area = c("Area1", "Area1")
  )

  expect_error(
    plot_timeseries(data, var = "value", time = "time", area = "area"),
...
# Warning when there are more than 15 areas and highlight is not specified
test_that("Warning when more than 15 areas without highlight", {
  data <- data.frame(
    time = rep(as.Date("2025-01-01"), 20),
    value = 5,
...
test_that("Correct handling of aggregation", {
  data <- data.frame(
    time = as.Date(c("2025-01-01", "2025-01-02", "2025-01-01", "2025-01-02")),
    value = c(1, 2, 2, 1),
    area = rep(1:2, each = 2)
  )
...
test_that("Correct time aggregation handling", {
  ...
  plot_result_week <- plot_timeseries(
    data,
    var = "var",
    time = "time",
    aggregate_time = "week"
  ...
  expect_equal(
    as.Date(c("2021-01-04", "2021-01-11")),
    plot_result_week$data$time
  )
})
...
test_that("Correct time aggregation handling", {
  ...
  plot_result_month <- plot_timeseries(
    data,
    var = "var",
    time = "time",
    aggregate_time = "month"
  ...
  expect_equal(
    as.Date(c("2021-01-01", "2021-02-01")),
    plot_result_month$data$time
  )
})
...
test_that("Highlight functionality for areas", {
  ...
  green_line_found <- "#168C81" %in%
    plot_build$plot$layers[[2]]$computed_geom_params$colour
...
test_that("Error when 'var' (cases) is missing or incorrect", {
  ...
  expect_error(
    plot_timeseries(data = data, time = "time", type = "counts", var = "cases"),
    "No column of the data matches the 'var' argument"
  )
...
test_that("Error when 'pop' is missing for 'incidence'", {
  ...
  expect_error(
    plot_timeseries(data = data, time = "time", var = "cases", type = "inc"),
    "pop' required if type = 'inc'"
  )
})


test_that("Duplicated time units are present in single time series", {
  data <- data.frame(
    time = as.Date(c("2022-01-01", "2022-01-01")),
    cases = c(100, 150),
    pop = c(1000, 1000)
  )
...
test_that("Correct time aggregation based on 'aggregate_time' - month", {
  ...
  expect_equal(length(unique(result$data$time)), 2)
})
...
test_that("Palette message when there is no grouping", {
  data <- data.frame(
    time = as.Date(c("2025-01-01", "2025-01-02", "2025-01-03", "2025-01-04")),
    cases = c(1, 2, 3, 1),
    pop = c(1000, 1000, 1000, 1000),
    area = rep(letters[1:2], each = 2)
  )
  expect_message(
    ...
    paste0(
      "A color ramp was selected for a single time series. ",
